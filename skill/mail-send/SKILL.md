---
name: mail-send
description: Send emails via the mail-send CLI tool. Use when the user asks to send an email, notify someone, mail a report or file, or trigger an email alert. Triggers on "send email", "mail", "notify", "email this", "send a message to", "send attachment".
---
...
```bash
df -h / | mail-send --to ops@example.com --subject "Disk report" --body -
```

**HTML**
```bash
mail-send --to team@example.com --subject "Report" \
  --body "<h1>Q1 Report</h1><p>Revenue up 12%</p>" --html
```

**Attachment** — repeat `--attach` for multiple files
```bash
mail-send --to user@example.com --subject "Logs" \
  --body "see attached" --attach /var/log/app.log
```

**Multiple recipients + CC**
```bash
mail-send --to alice@ex.com,bob@ex.com --cc boss@ex.com \
  --subject "Update" --body "Done"
```

**JSON output** — add `--json` for programmatic result parsing
```bash
mail-send --to user@example.com --subject "Alert" --body "Disk 95%" --json
...
```bash
mail-send --to a@b.com --subject "Hi" --body "ok" \
...
| 2 | Missing config | `mail-send init` then edit `~/.config/mail-send/config.json` |
...
Config file search: `./mail-send.json` > `~/.config/mail-send/config.json` > `~/.mail-send.json`. Use `--config <path>` to override.
